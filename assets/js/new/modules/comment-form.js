...
class CommentForm {
  constructor(container, header, options = {}) {
    ...
    this.parentSelect = this.form.querySelector(
      "#babdelaura_blogbundle_commentaire_parent"
    );
    this.parentOptions = [...this.parentSelect.querySelectorAll("option")];

    const notificationContainerSelector = container.getAttribute(
      "data-comment-form-notification"
    );
    const notificationContainer = document.querySelector(
      notificationContainerSelector
    );
  ...
  setParent(parentId) {
    ...
    const selectedOption = this.parentOptions
      .filter(option => option.value == parentId)
      .pop();
  ...
  restoreInOriginalContainer() {
    ...
    setTimeout(() => {
      ...
      const scrollTopLevel =
        offsetTop - offsetHeight - this.options.scrollTopDelta - 50;
  ...
  resetErrors() {
    const errorContainers = [
      ...this.container.querySelectorAll('[id^="error-"]')
    ];
